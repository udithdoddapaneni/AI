...
class AIPlayer:
    ...
    def get_alpha_beta_move(self, board):
        ...
        """
        p = sb.Popen("/home/udith/labs and projects/AI labs/connect4_l3_optimized/alpha_beta", stdout = sb.PIPE, stdin = sb.PIPE)
        p.stdin.write(bytes(str(self.player_number)+"\n", 'utf-8'))
        for i in range(6):
            for j in range(7):
                p.stdin.write(bytes(str(board[i][j])+"\n", 'utf-8'))
        p.stdin.flush()
        result = str(p.stdout.readline().strip())
        move = int(result[2:len(result)-1])
        return move
